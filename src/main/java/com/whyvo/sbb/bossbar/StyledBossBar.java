...
public class StyledBossBar<T extends BossBarStyle> extends BossBar {
	...
	public void setPercent(float percent) {
		if (percent != this.percent) {
            ...
            this.sendToPlayers(StyledBossBarS2CPayload.updateProgress(this));
		}
	}
    ...
    public void setBossBarStyle(BossBarStyleEntry<T> style) {
        if(this.bossBarStyle != style) {
            this.bossBarStyle = style;
            this.sendToPlayers(StyledBossBarS2CPayload.updateStyle(this));
        }
    }
	...
	public StyledBossBar<T> setDarkenSky(boolean darkenSky) {
		if (darkenSky != this.darkenSky) {
            ...
            this.sendToPlayers(StyledBossBarS2CPayload.updateProperties(this));
		}
		return this;
	}
	...
	public void setName(Text name) {
		if (!Objects.equal(name, this.name)) {
			super.setName(name);
            this.sendToPlayers(StyledBossBarS2CPayload.updateName(this));
		}
	}

    public void addPlayer(ServerPlayerEntity player) {
        if (this.players.add(player) && this.visible) {
            StyledBossBarAPI.sendToPlayer(player, StyledBossBarS2CPayload.add(this));
        }
    }

    public void removePlayer(ServerPlayerEntity player) {
		if (this.players.remove(player) && this.visible) {
            StyledBossBarAPI.sendToPlayer(player, StyledBossBarS2CPayload.remove(this.getUuid()));
		}
	}
	...
	public void setVisible(boolean visible) {
		if (visible != this.visible) {
            ...
            this.sendToPlayers(visible ? StyledBossBarS2CPayload.add(this) : StyledBossBarS2CPayload.remove(this.getUuid()));
		}
	}

    private void sendToPlayers(StyledBossBarS2CPayload payload) {
        for(ServerPlayerEntity player : this.players) {
            StyledBossBarAPI.sendToPlayer(player, payload);
        }
    }
